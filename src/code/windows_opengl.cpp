...
{
    GLuint vertex_shader_id = opengl.glCreateShader(GL_VERTEX_SHADER);
    ...
    opengl.glShaderSource(vertex_shader_id, array_count(vertex_shader_code),
                   vertex_shader_code, 0);
    opengl.glCompileShader(vertex_shader_id);

    GLuint fragment_shader_id = opengl.glCreateShader(GL_FRAGMENT_SHADER);
    ...
    opengl.glShaderSource(fragment_shader_id, array_count(fragment_shader_code),
    ...
    if(!linked)
    {
        ...
        opengl.glGetShaderInfoLog(vertex_shader_id, sizeof(vertex_errors), &ignored, vertex_errors);
        opengl.glGetShaderInfoLog(fragment_shader_id, sizeof(fragment_errors), &ignored, fragment_errors);
        opengl.glGetProgramInfoLog(program_id, sizeof(program_errors), &ignored, program_errors);
...
opengl_init(Opengl_Info info)
{
    ...
    if(info.GL_EXT_framebuffer_sRGB)
    {
        ...
        opengl.glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE,
...
opengl_init_fbo(i32 window_width, i32 window_height)
{
	if(opengl.fbo != 0)
	{
		opengl.glDeleteFramebuffers(1, &opengl.fbo);
		glDeleteTextures(1, &opengl.fbo_texture);
		opengl.glDeleteRenderbuffers(1, &opengl.fbo_depth);
	}

	opengl.glGenFramebuffers(1, &opengl.fbo);
	...
	opengl.glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, opengl.fbo_texture, 0);

	// depth renderbuffer
	opengl.glGenRenderbuffers(1, &opengl.fbo_depth);
	opengl.glBindRenderbuffer(GL_RENDERBUFFER, opengl.fbo_depth);
	opengl.glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, window_width, window_height);
	opengl.glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, opengl.fbo_depth);
...
opengl_render(i32 window_width, i32 window_height, Memory_Arena* render_arena, GLuint buffer)
{
    ...
    opengl.glUniformMatrix4fv(opengl.transform_id, 1, GL_FALSE, ortho.e);
...
static void
opengl_post_process_and_render_to_screen()
{
	opengl.glBindFramebuffer(GL_FRAMEBUFFER, 0); // 0 = back to screen
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glDisable(GL_BLEND);
	opengl.glUseProgram(opengl.filter_program);
	opengl.glUniform1i(opengl.filter_texture_sampler_id, 0);
